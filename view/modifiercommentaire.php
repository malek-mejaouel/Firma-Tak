<?php
include_once 'C:/xampp/htdocs/projetagriculture/Controller/commentairec.php';
include_once 'C:/xampp/htdocs/projetagriculture/Model/commentaire.php';
...
if (isset($_GET['id_c']) && !empty($_GET['id_c'])) {
    $id_c = $_GET['id_c'];
    $rec = $commentairec->recuperercommentaire($id_c);

    if ($rec) {
        // Formulaire soumis en méthode POST
        if ($_SERVER['REQUEST_METHOD'] === 'POST' && isset($_POST['contenu'], $_POST['date'], $_POST['id_post'])) {
            if (!empty($_POST['contenu']) && !empty($_POST['date']) && !empty($_POST['id_post'])) {
                // Création d'un objet commentaire
                $commentaire = new commentaire($_POST['contenu'], $_POST['date'], $_POST['id_post']);
                $commentairec->modifier($commentaire, $id_c);
    ...
    $error = "Identifiant du commentaire manquant.";
}
?>
...
.row {
    ...
    <h2 class="text-center mb-4">Modifier Commentaire</h2>

    <?php if ($rec) : ?>
        <form method="POST" onsubmit="return verif();">
            <div class="form-group row mb-3">
                <label for="contenu" class="col-sm-3 col-form-label">Nouveau commentaire</label>
                <div class="col-sm-9">
                    <input type="text" name="contenu" 
                           value="<?= htmlspecialchars($rec['contenu']); ?>" 
                           class="form-control" id="contenu" 
            ...
            <div class="form-group row mb-3">
                ...
                <div class="col-sm-9">
                    <input type="date" name="date" 
                           value="<?= htmlspecialchars($rec['date']); ?>" 
                           class="form-control" id="date" 
                           placeholder="Date" required>
                </div>
            </div>
            <div class="form-group row mb-3">
                <label for="id_post" class="col-sm-3 col-form-label">Publication associée</label>
                <div class="col-sm-9">
                    <input type="text" name="id_post" 
                           value="<?= htmlspecialchars($rec['id_post']); ?>" 
                           class="form-control" id="id_post" 
                           placeholder="ID de la publication" required>
                </div>
            </div>
    ...
    <?php else : ?>
        <div class="alert alert-warning text-center">
            Aucun commentaire trouvé pour modification.
        </div>
    <?php endif; ?>
</div>
	...
	<script>
        function verif() {
            ...
            var id_post = document.getElementById("id_post").value.trim();
            if (contenu === "" || date === "" || id_post === "") {
